...
/**
 * Maintain a map of resolved files.
 */
...
function compileDependency (tree: DependencyTree, name: string, isBrowser: boolean): Promise<string> {
  ...
  const override: Browser = isBrowser ? (tree.typings.browser || tree.browser) : tree.typings.main
  const mainPath = typeof override === 'string' ? override : tree.main

  if (!mainPath) {
    return Promise.resolve(undefined)
  }

  const entry = resolveFrom(tree.src, normalizePath(mainPath))
  const overrides: Overrides = typeof override === 'string' ? {} : override

  return compileDependencyDeclaration(tree, name, entry, overrides, dependencies, isBrowser)
}
...
/**
 * Normalize Windows to Unix paths.
 */
